<template>
	<view>
		...
		<view class="paddingLeftRight">
			<view class="rechargeInfo borderRadius">
				...
				<view class="cent content">
					...
					<text class="number fontWeight orangeColor">{{amount}}</text>
		...
		<button class="submitBtnOrange absoluteBtn" @tap="go">打印数据</button>
	</view>
</template>
...
<script>
	...
	export default{
		...
		methods:{
			...
			go(){
				uni.redirectTo({
					url: '/pages/annal/RechargeOrderPrint/RechargeOrderPrint?rechargeId='+this.rechargeId,
				});
			},
			//时间戳转时间年月日
